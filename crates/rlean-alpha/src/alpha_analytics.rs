...
mod tests {
    ...
    fn ranking_keeps_short_side_alpha_when_signed_transfer_is_positive() {
        ...
        let symbols: Vec<Symbol> = (0..6)
            .map(|i| Symbol::create_equity(&format!("S{i}"), &rlean_core::Market::usa()))
            .collect();

        let mut expired = Vec::new();
        for p in 0..6 {
            let ts = rlean_core::NanosecondTimestamp::from_secs(1_700_000_000 + p);
            ...
            for (k, sym) in symbols.iter().enumerate() {
                ...
                let mut ins = Insight::new(
                    sym.clone(),
                    InsightDirection::Up,
                    rlean_core::TimeSpan::from_days(1),
