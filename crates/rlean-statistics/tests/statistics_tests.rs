use rlean_statistics::statistics::Statistics;
...
#[test]
fn sharpe_ratio_uses_lean_annual_performance() {
    let returns = vec![dec!(0.01), dec!(-0.005), dec!(0.012), dec!(0.003)];
    let sharpe = Statistics::sharpe_ratio(&returns, dec!(0.04));

    assert!(
        (sharpe - dec!(20.235554543753853)).abs() < dec!(0.000000000001),
        "expected C# LEAN-compatible annual-performance Sharpe, got {sharpe}"
    );
}
