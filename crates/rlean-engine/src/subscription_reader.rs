...
use rlean_storage::{FactorFileEntry, MarketPartitionDayQuery, OptionEodBar, QueryParams};
...
pub struct SubscriptionStream {
    ...
    exhausted: bool,
    producer_error: Option<rlean_core::LeanError>,
}
...
impl SubscriptionProducerState {
    fn new(
        ...
        let factor_rows = if config.normalization_mode != rlean_core::DataNormalizationMode::Raw {
        ...
        if config.normalization_mode != rlean_core::DataNormalizationMode::Raw
            && matches!(
            ...
            if self.custom_overflow.is_empty() {
                ...
                if self.config.data_kind != SubscriptionDataKind::Market {
                    ...
                    agent_debug_log(
                        "initial",
                        "H1,H2,H5",
                        "crates/rlean-engine/src/subscription_reader.rs:384",
                ...
                if self.config.data_kind != SubscriptionDataKind::Market {
                    agent_debug_log(
                        "initial",
                        "H1,H2",
                        "crates/rlean-engine/src/subscription_reader.rs:386",
        ...
        for point in points {
                    ...
                    .unwrap_or(false)
            {
                agent_debug_log(
                    "initial",
                    "H4",
                    "crates/rlean-engine/src/subscription_reader.rs:551",
    ...
    async fn local_market_days(
        ...
        let table = if self.config.resolution.is_tick() {
            rlean_storage::iceberg_store::MARKET_TICKS
        } else if self.config.tick_type == TickType::Quote {
            rlean_storage::iceberg_store::MARKET_QUOTE_BARS
        } else {
            rlean_storage::iceberg_store::MARKET_TRADE_BARS
        };
        Ok(self
            .context
            .store
    ...
    /// `rlean_storage::iceberg_store::market_day_values`). Keying the uncached-day
    ...
    async fn load_trade_bars(
        ...
        let grouped = self
            ...
            .await
            .map_err(|e| rlean_core::LeanError::DataError(e.to_string()))?;
        let bars = grouped
            .get(&self.config.symbol.id.sid)
            .cloned()
            .unwrap_or_default();
    ...
    async fn load_option_partition(&mut self) -> LeanResult<LoadedPartition> {
        ...
        while date <= window_end {
            ...
            if !rows.is_empty() {
                ...
                .map_err(|error| rlean_core::LeanError::DataError(error.to_string()))?
                {
    ...
    async fn load_custom_partition(&mut self) -> LeanResult<LoadedPartition> {
        if !self.custom_overflow.is_empty() {
            ...
            agent_debug_log(
                "initial",
                "H1,H2",
                "crates/rlean-engine/src/subscription_reader.rs:1438",
        ...
        agent_debug_log(
            "initial",
            "H3,H5",
            "crates/rlean-engine/src/subscription_reader.rs:1477",
        ...
        if out.len() > CUSTOM_PARTITION_STAGE_CHUNK_ROWS {
            ...
            agent_debug_log(
                "initial",
                "H1,H3",
                "crates/rlean-engine/src/subscription_reader.rs:1680",
    ...
    async fn fetch_custom_window_if_missing(
        &mut self,
        custom: &rlean_data::CustomSubscriptionMetadata,
        window_start: chrono::NaiveDate,
        window_end: chrono::NaiveDate,
    ...
    fn is_regular_market_bar(&self, end_time: DateTime) -> bool {
        if self.config.resolution == rlean_core::Resolution::Daily {
            return true;
        }
...
async fn read_custom_source_points(
    ...
    match data_source.format {
        ...
        CustomDataFormat::Parquet => {
            ...
            rlean_storage::provider_parquet_bytes_to_custom_points(
                &bytes,
                date,
                &data_source.uri,
                &value_column_refs,
...
mod tests {
    ...
    use rlean_storage::{FactorFileEntry, IcebergStore, MapFileEntry};
