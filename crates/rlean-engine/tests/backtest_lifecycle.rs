...
use rlean_data_providers::{DataType, HistoryRequest, IHistoryProvider};
use rlean_engine::{
    algorithm_manager::{AlgorithmManager, OrderEventProcessing},
    AlgorithmRuntimeContext,
};
use rlean_engine::{runner::backtest::run_backtest, BacktestRunConfig};
use rlean_orders::{
...
use rlean_statistics::TradeBuilder;
...
async fn backtest_runner_delivers_lifecycle_callbacks_to_sdk_bridge() {
    ...
    let result = run_backtest(
        ...
        BacktestRunConfig {
            ...
            data_feed_options: rlean_engine::data_feed::DataFeedOptions::default(),
            output_dir: None,
            artifact_sink: None,
            progress: None,
        },
...
async fn backtest_runner_replays_warmup_before_warmup_finished() {
    ...
    algorithm
        ...
        .lock()
        .unwrap()
        .set_warm_up(rlean_core::TimeSpan::ONE_DAY);
    let subscription_symbol = algorithm.subscriptions()[0].symbol.clone();
    let algorithm_state = algorithm.algorithm.clone();
...
fn algorithm_manager_dispatches_auxiliary_slice_callbacks_before_on_data() {
    ...
    let mut services = rlean_algorithm::NoopAlgorithmServices::default();
...
fn algorithm_manager_dispatches_order_event_after_fill_settlement() {
    ...
    slice.add_bar(TradeBar::new(
        symbol.clone(),
        time,
        rlean_core::TimeSpan::ONE_MINUTE,
