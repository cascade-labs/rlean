...
use crate::thetadata_models::*;

const BASE_URL: &str = "https://thetadata.cascadelabs.io";
...
impl ThetaDataClient {
    ...
    ///
    /// Uses `expiration="*"` to retrieve all expirations for that date.
    ///
    /// Results are stored in a structured Parquet file at:
    /// `{data_root}/option/usa/daily/{root_lower}/{root_lower}_eod.parquet`
    ///
    /// All dates for the underlying live in one file.  On cache hit (the file
    /// contains rows for `date`) the API is skipped and rows are read from
    /// Parquet.  On cache miss the API is called and new rows are appended
    /// (merge-write) to the Parquet file before being returned.
    pub async fn get_option_eod_for_date(
        &self,
        root: &str,
        date: NaiveDate,
    ) -> Result<Vec<V3OptionEod>> {
        let parquet_path = option_eod_path(&self.data_root, root);
        let date_ns = date_to_ns(date);

        // ── Cache hit ────────────────────────────────────────────────────────
        if self.parquet_reader.option_eod_date_cached(&parquet_path, date_ns) {
            debug!(
                "ThetaData: Parquet cache hit for {root} on {date} at {}",
                parquet_path.display()
            );
            let cached = self
                .parquet_reader
                .read_option_eod_bars_for_date(&parquet_path, date_ns)?;
        ...
        if !api_rows.is_empty() {
            // Convert → OptionEodBar, read existing Parquet, merge, write back.
            let new_bars: Vec<OptionEodBar> = v3_to_option_eod_bars(root, date, &api_rows);

            let existing: Vec<OptionEodBar> = if parquet_path.exists() {
                self.parquet_reader
            ...
            // De-duplicate by (date_ns, symbol_value).
            merged.sort_by(|a, b| {
                a.date.cmp(&b.date).then_with(|| a.symbol_value.cmp(&b.symbol_value))
            });
            merged.dedup_by(|a, b| a.date == b.date && a.symbol_value == b.symbol_value);

            if let Err(e) = self.parquet_writer.write_option_eod_bars(&merged, &parquet_path) {
                warn!("ThetaData: failed to write option EOD Parquet for {root}: {e}");
            } else {
                debug!(
    ...
    async fn execute<T: DeserializeOwned>(
        ...
        let mut query = format!(
            "{BASE_URL}{API_VERSION}/{}?format=ndjson",
...
mod tests {
    ...
    fn test_url_stock_eod_endpoint() {
        ...
        assert!(url.starts_with("https://thetadata.cascadelabs.io/v3/stock/history/eod"));
    ...
    fn test_option_eod_parquet_write_and_read() {
        // Mirrors ThetaDataDownloaderTests: verify that option EOD bars can be
        // written to Parquet and read back filtered by date.
        use lean_storage::{ParquetWriter, ParquetReader, WriterConfig, OptionEodBar, option_eod_path};
        use rust_decimal::Decimal;
        use std::str::FromStr;
        ...
        let path = option_eod_path(root, "SPY");
        ...
        // Read back for the specific date.
        let date_ns = date_to_ns(date);
        let reader = ParquetReader::new();
        let rows = reader
            .read_option_eod_bars_for_date(&path, date_ns)
            .expect("read");
    ...
    fn test_option_eod_parquet_cache_miss_for_different_date() {
        // Writing bars for date A must NOT be visible when querying date B.
        use lean_storage::{ParquetWriter, ParquetReader, WriterConfig, OptionEodBar, option_eod_path};
        use rust_decimal::Decimal;
        use tempfile::TempDir;
        ...
        let path = option_eod_path(root, "SPY");
        let writer = ParquetWriter::new(WriterConfig::default());
        writer.write_option_eod_bars(&[bar], &path).expect("write");

        let reader = ParquetReader::new();
        let date_b_ns = date_to_ns(date_b);
        assert!(
            !reader.option_eod_date_cached(&path, date_b_ns),
            "date B should not be in cache when only date A was written"
        );
    }

    #[test]
    fn test_option_eod_parquet_path_layout() {
        // Mirrors LEAN's path convention: option/usa/daily/{underlying}/{underlying}_eod.parquet
        use lean_storage::option_eod_path;
        use std::path::Path;
        ...
        assert_eq!(
            path.to_str().unwrap(),
            "/data/option/usa/daily/spy/spy_eod.parquet"
        );

        let path2 = option_eod_path(root, "QQQ");
        assert_eq!(
            path2.to_str().unwrap(),
            "/data/option/usa/daily/qqq/qqq_eod.parquet"
        );
    }

    #[test]
    fn test_option_eod_date_not_cached_when_file_missing() {
        use lean_storage::{ParquetReader, option_eod_path};
        use std::path::Path;
        let reader = ParquetReader::new();
        let path = option_eod_path(Path::new("/nonexistent"), "SPY");
        let date_ns = date_to_ns(NaiveDate::from_ymd_opt(2021, 4, 19).unwrap());
        assert!(!reader.option_eod_date_cached(&path, date_ns));
    }
}
