...
impl Indicator for Stochastic {
    ...
    fn is_ready(&self) -> bool { self.window.is_full() && self.d_sma.is_ready() }
    fn current(&self) -> IndicatorResult { self.current.clone() }
    fn samples(&self) -> usize { self.samples }
    fn warm_up_period(&self) -> usize { self.k_period + self.d_period - 1 }
    ...
    fn update_bar(&mut self, bar: &lean_data::TradeBar) -> IndicatorResult {
        ...
        if self.window.is_full() {
            let highest_high = self.window.iter().map(|(h, _, _)| *h).fold(dec!(0), |acc, x| acc.max(x));
            let lowest_low   = self.window.iter().map(|(_, l, _)| *l).fold(Price::MAX, |acc, x| acc.min(x));
            let range = highest_high - lowest_low;

            self.k = if range.is_zero() { dec!(50) } else {
