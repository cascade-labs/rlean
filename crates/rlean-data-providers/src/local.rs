...
use rlean_core::{Resolution, Symbol, TickType};
use rlean_data::{MarginInterestRate, QuoteBar, Tick, TradeBar};
use rlean_storage::{IcebergStore, OptionEodBar, OptionUniverseRow, QueryParams};
...
fn expected_daily_dates(
    ...
    let hours = rlean_core::MarketHoursDatabase::global().exchange_hours(symbol);
...
impl IHistoryProvider for LocalHistoryProvider {
    ...
    async fn get_perpetual_contexts(
        &self,
        request: &HistoryRequest,
    ) -> anyhow::Result<Vec<rlean_data::PerpetualContext>> {
        let mut params = QueryParams::new().with_time_range(request.start, request.end);
        params.predicate = params.predicate.with_symbols(vec![request.symbol.id.sid]);
        Ok(self
            .store
    ...
    async fn get_option_universes(
        ...
        tickers: &[String],
        date: chrono::NaiveDate,
    ) -> anyhow::Result<HashMap<String, Vec<rlean_storage::OptionUniverseRow>>> {
