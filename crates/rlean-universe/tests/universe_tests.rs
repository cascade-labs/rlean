...
mod manual_universe_tests {
    ...
    use rlean_universe::universe::{ManualUniverse, Universe, UniverseSettings};
    ...
    fn universe_settings_default_resolution_is_daily() {
        ...
        assert_eq!(settings.resolution, rlean_core::Resolution::Daily);
...
mod coarse_universe_tests {
    use super::*;
    use rlean_universe::coarse_fundamental::FnCoarseFilter;
...
mod fine_fundamental_tests {
    use super::*;
    use rlean_universe::{FineFundamental, FineFundamentalUniverseSelectionModel};
...
mod liquid_universe_tests {
    use super::*;
    use rlean_universe::LiquidUniverseSelectionModel;
...
mod market_cap_universe_tests {
    use super::*;
    use rlean_universe::MarketCapUniverseSelectionModel;
...
mod sector_universe_tests {
    use super::*;
    use rlean_universe::SectorUniverseSelectionModel;
...
mod etf_universe_tests {
    use super::*;
    use rlean_universe::{EtfConstituent, EtfConstituentsUniverse, EtfUniverses};
...
mod scheduled_universe_tests {
    ...
    use rlean_universe::{ScheduledUniverseSelectionModel, UniverseSchedule};
...
mod ema_cross_universe_tests {
    use rlean_core::{Market, Symbol};
    use rlean_universe::{CoarseFundamental, EmaCrossUniverseSelectionModel};
    use rust_decimal::Decimal;
    ...
    fn flat_price_no_bullish_cross() {
        ...
        assert!(
            last.is_empty(),
            "flat price should not trigger a bullish cross"
        );
    }
...
mod inception_date_universe_tests {
    use chrono::NaiveDate;
    use rlean_universe::{InceptionDateUniverseSelectionModel, LiquidEtfUniverse};
    ...
    fn from_pairs_builds_correctly() {
        let pairs = vec![
            (
                "SPY".to_string(),
                NaiveDate::from_ymd_opt(1993, 1, 22).unwrap(),
            ),
            (
                "QQQ".to_string(),
                NaiveDate::from_ymd_opt(1999, 3, 10).unwrap(),
            ),
        ];
...
mod option_universe_tests {
    use chrono::NaiveDate;
    use rlean_universe::{OptionContractView, OptionRight, OptionUniverseSelectionModel};
    use rust_decimal_macros::dec;
    ...
    fn delta_filter_excludes_out_of_range() {
        let model = OptionUniverseSelectionModel::new(0, 45).with_delta(dec!(0.20), dec!(0.50));
        let contracts = vec![
    ...
    fn right_filter_only_puts() {
        let model = OptionUniverseSelectionModel::new(0, 45).with_right(OptionRight::Put);
        let contracts = vec![
    ...
    fn strike_filter_excludes_out_of_range() {
        let model = OptionUniverseSelectionModel::new(0, 45).with_strike(dec!(390.0), dec!(420.0));
        let contracts = vec![
    ...
    fn contracts_without_delta_excluded_when_delta_filter_set() {
        ...
        assert_eq!(
            result.len(),
            1,
            "no-delta contracts pass through delta filter"
        );
    }
}
