...
/// Directories skipped only at the root of the strategy project. These are the
/// rlean project layout's own output directories, never algorithm source.
///
/// `live` is load-bearing: deployment directories are created at
/// `<project>/live/<deploy_id>`, so copying it would drag every previous
/// deployment - including the snapshot currently being written - into the new
/// one. A nested subpackage legitimately named `live` deeper in the project is
/// still shipped.
const SNAPSHOT_EXCLUDED_ROOT_DIRS: &[&str] = &["live", "backtests", "research_runs", "research"];

/// Directories skipped at any depth: Python/VCS/virtualenv artifacts that are
/// regenerated by the container and must not ship with the algorithm.
const SNAPSHOT_EXCLUDED_DIRS: &[&str] = &["__pycache__", ".git", ".venv", "venv"];

/// Compiled Python skipped at any depth; the container recompiles from source.
const SNAPSHOT_EXCLUDED_EXTENSIONS: &[&str] = &["pyc"];

/// A strategy project is source code, so anything past this is a stray data or
/// artifact directory the user probably did not mean to deploy. Warn but ship.
const SNAPSHOT_WARN_BYTES: u64 = 16 * 1024 * 1024;

/// Past this the snapshot is copied on every launch, upgrade and resume, which
/// is a real cost and almost certainly a mistake. Refuse rather than silently
/// balloon the deployment.
const SNAPSHOT_MAX_BYTES: u64 = 128 * 1024 * 1024;

/// One file to copy into the snapshot: absolute source path plus its path
...
enum SnapshotSizeVerdict {
    Ship,
    Warn,
    Refuse,
}

fn snapshot_size_verdict(total_bytes: u64) -> SnapshotSizeVerdict {
    if total_bytes > SNAPSHOT_MAX_BYTES {
        SnapshotSizeVerdict::Refuse
    } else if total_bytes > SNAPSHOT_WARN_BYTES {
        SnapshotSizeVerdict::Warn
    } else {
        SnapshotSizeVerdict::Ship
    }
}

/// Snapshot the strategy's containing directory into `<deployment_dir>/code`,
/// preserving the relative layout, and return the entry file inside the copy.
///
/// LEAN's unit of deployment is the algorithm's project directory, not the
/// entry file. `Queues/JobQueue.cs:279` resolves the algorithm file and then
/// passes its containing directory to
/// `PythonInitializer.AddAlgorithmLocationPath`, which
/// (`Common/Python/PythonInitializer.cs:160`) asserts the path is a directory
/// and prepends it to `sys.path`. Sibling modules import because that whole
/// directory is present and on the path, so the snapshot must carry it.
fn snapshot_strategy_code(strategy: &Path, deployment_dir: &Path) -> Result<PathBuf> {
    let source_dir = strategy
        .parent()
        .filter(|parent| !parent.as_os_str().is_empty())
        .unwrap_or_else(|| Path::new("."));
    let entry_name = strategy
        .file_name()
        .ok_or_else(|| anyhow::anyhow!("strategy path {} has no file name", strategy.display()))?
        .to_owned();

    let code_dir = deployment_dir.join("code");
    // Re-snapshot from scratch so `live upgrade` and `live resume` mirror the
    // source project exactly instead of keeping files deleted upstream.
    if code_dir.exists() {
        std::fs::remove_dir_all(&code_dir)
            .with_context(|| format!("failed to clear {}", code_dir.display()))?;
    }
    std::fs::create_dir_all(&code_dir)
        .with_context(|| format!("failed to create {}", code_dir.display()))?;
    let code_root = std::fs::canonicalize(&code_dir)
        .with_context(|| format!("canonicalize {}", code_dir.display()))?;

    let files = plan_strategy_snapshot(source_dir, &code_root)?;
    let total_bytes: u64 = files
        ...
        .sum();
    match snapshot_size_verdict(total_bytes) {
        SnapshotSizeVerdict::Refuse => bail!(
            "strategy project {} is {} and exceeds the {} deployment snapshot limit; \
             move data or artifacts out of the project directory",
            source_dir.display(),
            format_bytes(total_bytes),
            format_bytes(SNAPSHOT_MAX_BYTES)
        ),
        SnapshotSizeVerdict::Warn => eprintln!(
            "Warning: strategy project {} snapshots to {} across {} files; \
             every launch, upgrade and resume copies it",
            source_dir.display(),
            format_bytes(total_bytes),
            files.len()
        ),
    ...
    for file in &files {
        let dest = code_dir.join(&file.relative);
        if let Some(parent) = dest.parent() {
            std::fs::create_dir_all(parent)
                .with_context(|| format!("failed to create {}", parent.display()))?;
        }
        std::fs::copy(&file.source, &dest).with_context(|| {
            format!(
                "failed to copy {} to {}",
                file.source.display(),
                dest.display()
    ...
    let dest = code_dir.join(&entry_name);
    if !dest.is_file() {
        bail!(
            "strategy entry file {} was not captured in the deployment snapshot at {}",
            strategy.display(),
            dest.display()
        );
    }
...
/// Walk the strategy project and list the files that belong in the snapshot.
///
/// `code_root` is the canonical snapshot destination. Any directory that is the
/// destination or an ancestor of it is pruned, so the walk can never descend
/// into the tree it is writing, whatever the project layout. Combined with not
/// following directory symlinks, that makes the walk finite.
fn plan_strategy_snapshot(source_dir: &Path, code_root: &Path) -> Result<Vec<SnapshotFile>> {
    let mut files = Vec::new();
    let mut stack = vec![(source_dir.to_path_buf(), PathBuf::new())];
    while let Some((dir, relative)) = stack.pop() {
        ...
        for entry in entries {
            ...
            if file_type.is_symlink() {
                // Follow symlinks to files (the target's bytes ship), but never
                // to directories: a symlink can point back up the tree and the
                // walk would not terminate.
                let target = std::fs::metadata(&path)
                    .with_context(|| format!("failed to resolve symlink {}", path.display()))?;
                if target.is_dir() {
                    eprintln!(
                        "Warning: skipping symlinked directory {} in strategy snapshot",
                        path.display()
                    );
                    continue;
                }
                if !is_excluded_snapshot_file(&name_str) {
            ...
            if file_type.is_dir() {
                if SNAPSHOT_EXCLUDED_DIRS.contains(&name_str.as_ref())
                    || (relative.as_os_str().is_empty()
                        && SNAPSHOT_EXCLUDED_ROOT_DIRS.contains(&name_str.as_ref()))
                {
                    continue;
                }
                if leads_to_snapshot_destination(&path, code_root) {
                    continue;
                }
                stack.push((path, entry_relative));
                continue;
            }
...
fn is_excluded_snapshot_file(name: &str) -> bool {
    Path::new(name)
        .extension()
        .and_then(|ext| ext.to_str())
        .is_some_and(|ext| SNAPSHOT_EXCLUDED_EXTENSIONS.contains(&ext))
}

/// True when `dir` is the snapshot destination or contains it, which is the
/// structural guard against a deployment directory nested inside the project
/// being copied into itself.
fn leads_to_snapshot_destination(dir: &Path, code_root: &Path) -> bool {
    let Ok(canonical) = std::fs::canonicalize(dir) else {
        return false;
    };
    code_root.starts_with(&canonical)
}
...
mod tests {
    ...
    fn snapshot_test_root(label: &str) -> PathBuf {
        let root = std::env::temp_dir().join(format!(
            "rlean-snapshot-{label}-{}-{:?}",
            std::process::id(),
    ...
    /// Every file in `dir`, relative to it, sorted.
    ...
    #[test]
    fn test_snapshot_copies_project_directory_with_subpackages() {
        let root = snapshot_test_root("project");
        let project = root.join("uw_control_rh");
        write_file(&project.join("main.py"), "import live_flow_alerts\n");
        write_file(&project.join("live_flow_alerts.py"), "ALERTS = 1\n");
        write_file(&project.join("config.json"), "{}\n");
        write_file(&project.join("alphas/__init__.py"), "");
        write_file(&project.join("alphas/tide/__init__.py"), "");
        write_file(&project.join("alphas/tide/signal.py"), "SIGNAL = 2\n");

        let deployment_dir = root.join("deploy");
        std::fs::create_dir_all(&deployment_dir).unwrap();
        let entry = snapshot_strategy_code(&project.join("main.py"), &deployment_dir).unwrap();

        assert_eq!(entry, deployment_dir.join("code").join("main.py"));
        assert!(entry.is_file());
        assert_eq!(
            list_relative_files(&deployment_dir.join("code")),
            vec![
                "alphas/__init__.py",
                "alphas/tide/__init__.py",
                "alphas/tide/signal.py",
                "config.json",
                "live_flow_alerts.py",
                "main.py",
            ]
    ...
    #[test]
    fn test_snapshot_excludes_artifact_dirs_and_never_recurses_into_nested_live() {
        let root = snapshot_test_root("nested-live");
        let project = root.join("uw_control_rh");
        write_file(&project.join("main.py"), "entry\n");
        write_file(&project.join("live_flow_alerts.py"), "alerts\n");
        // A subpackage legitimately named `live` deeper in the tree must survive;
        // only the project-root `live/` is the deployment output directory.
        write_file(&project.join("alphas/live/__init__.py"), "");
        write_file(&project.join("backtests/2026/result.json"), "{}\n");
        write_file(&project.join("research_runs/run.json"), "{}\n");
        write_file(&project.join("research/notes.ipynb"), "{}\n");
        write_file(&project.join("__pycache__/main.cpython-311.pyc"), "junk\n");
        write_file(&project.join(".git/HEAD"), "ref\n");
        write_file(&project.join(".venv/pyvenv.cfg"), "cfg\n");
        write_file(&project.join("venv/pyvenv.cfg"), "cfg\n");
        write_file(&project.join("alphas/stale.pyc"), "junk\n");
        // A previous deployment already sitting under the project root.
        write_file(
            &project.join("live/2026-08-19_000000_uw_control_rh/code/main.py"),
            "old\n",
        );

        // The deployment directory nests inside the strategy project, exactly as
        // `reserve_live_dir` creates it.
        let deployment_dir = project.join("live/2026-08-20_005028_uw_control_rh");
        std::fs::create_dir_all(&deployment_dir).unwrap();
        let entry = snapshot_strategy_code(&project.join("main.py"), &deployment_dir).unwrap();
        ...
        assert_eq!(
            list_relative_files(&deployment_dir.join("code")),
            vec!["alphas/live/__init__.py", "live_flow_alerts.py", "main.py",]
        );
        // Nothing from the deployment tree was copied into itself.
        assert!(!deployment_dir.join("code/live").exists());
        assert!(!deployment_dir.join("code/code").exists());

        let _ = std::fs::remove_dir_all(&root);
    }

    #[test]
    fn test_snapshot_prunes_deployment_dir_nested_outside_live() {
        // The `live/` name rule is not the only thing preventing recursion: any
        // directory on the path to the snapshot destination is pruned, so a
        // deployment directory nested anywhere under the project is still safe.
        let root = snapshot_test_root("nested-elsewhere");
        let project = root.join("proj");
        write_file(&project.join("main.py"), "entry\n");
        write_file(&project.join("helper.py"), "helper\n");

        let deployment_dir = project.join("deployments/deploy-1");
        std::fs::create_dir_all(&deployment_dir).unwrap();
        write_file(&deployment_dir.join("deployment.json"), "{}\n");
        snapshot_strategy_code(&project.join("main.py"), &deployment_dir).unwrap();

        let copied = list_relative_files(&deployment_dir.join("code"));
        assert_eq!(copied, vec!["helper.py", "main.py"]);
        assert!(!deployment_dir.join("code/deployments").exists());
    ...
    #[cfg(unix)]
    #[test]
    fn test_snapshot_does_not_follow_directory_symlink_loop() {
        let root = snapshot_test_root("symlink-loop");
        let project = root.join("proj");
        write_file(&project.join("main.py"), "entry\n");
        write_file(&project.join("pkg/mod.py"), "mod\n");
    ...
    #[test]
    fn test_snapshot_redeploy_mirrors_source_project() {
        // `live upgrade` re-snapshots into the same deployment directory; the
        // result must match the source rather than accumulate stale files.
        let root = snapshot_test_root("redeploy");
        let project = root.join("proj");
        write_file(&project.join("main.py"), "v1\n");
        write_file(&project.join("removed.py"), "gone soon\n");
        ...
        assert_eq!(
            list_relative_files(&deployment_dir.join("code")),
            vec!["main.py", "removed.py"]
        );

        std::fs::remove_file(project.join("removed.py")).unwrap();
        write_file(&project.join("main.py"), "v2\n");
        write_file(&project.join("added.py"), "new\n");
        let entry = snapshot_strategy_code(&project.join("main.py"), &deployment_dir).unwrap();

        assert_eq!(
            list_relative_files(&deployment_dir.join("code")),
            vec!["added.py", "main.py"]
        );
        assert_eq!(std::fs::read_to_string(&entry).unwrap(), "v2\n");

        let _ = std::fs::remove_dir_all(&root);
    }

    #[test]
    fn test_snapshot_size_verdict_thresholds() {
        assert_eq!(snapshot_size_verdict(0), SnapshotSizeVerdict::Ship);
        assert_eq!(
            snapshot_size_verdict(SNAPSHOT_WARN_BYTES),
            SnapshotSizeVerdict::Ship
        );
        assert_eq!(
            snapshot_size_verdict(SNAPSHOT_WARN_BYTES + 1),
            SnapshotSizeVerdict::Warn
        );
        assert_eq!(
            snapshot_size_verdict(SNAPSHOT_MAX_BYTES),
            SnapshotSizeVerdict::Warn
        );
        assert_eq!(
            snapshot_size_verdict(SNAPSHOT_MAX_BYTES + 1),
            SnapshotSizeVerdict::Refuse
        );
    }
