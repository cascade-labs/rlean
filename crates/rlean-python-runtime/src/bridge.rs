...
use rlean_algorithm::algorithm::{AlgorithmStatus, DataDeliveryPayload, SecurityChanges};
...
use rlean_sdk::data::{SharedSliceFrame, SliceView};
use rlean_sdk::orders::{
    assignment_expiry_event_view, margin_call_payload, margin_call_requests_from_orders,
    otm_expiry_event_view, OrderEventView,
};
use rlean_sdk::securities::SymbolHandle;
use rlean_sdk::universe::SecurityChangesView;
...
impl PythonAlgorithmBridge {
    pub fn from_strategy(
        ...
        rlean_sdk::python_framework::register_python_strategy_object(
            rlean_sdk::python_framework::custom_universe_state_key(&state),
...
impl LifecycleBridge for PythonAlgorithmBridge {
    fn initialize(&mut self, _services: &mut dyn AlgorithmServices) -> Result<()> {
        debug_log_sigint_state(
            "pre-fix",
            "H5",
            "crates/rlean-python-runtime/src/bridge.rs:before_strategy_initialize",
        );
        let result = self.call_method0_if_present_any(&["initialize", "Initialize"]);
        debug_log_sigint_state(
            "pre-fix",
            "H5",
            "crates/rlean-python-runtime/src/bridge.rs:after_strategy_initialize",
        );
        result
    }
...
mod tests {
    ...
    fn load_test_strategy_bridge(path: &Path) -> PythonAlgorithmBridge {
        ...
        let runtime_context = rlean_engine::AlgorithmRuntimeContext::with_history_service(
            Arc::new(rlean_algorithm::lifecycle::NullHistoryService),
            HashMap::new(),
        );
...
"#,
        ...
        let option_symbol = Symbol::create_option(
            ...
            dec!(100),
            rlean_core::OptionRight::Put,
            rlean_core::OptionStyle::American,
        );
        let mut chain =
            rlean_options::OptionChain::new(subscription.canonical.clone(), dec!(102.5));
        chain.add_contract(rlean_options::OptionContract::new(option_symbol));
