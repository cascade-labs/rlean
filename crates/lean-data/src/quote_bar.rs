...
impl QuoteBar {
    ...
    pub fn update(&mut self, bid: Price, ask: Price, bid_size: Quantity, ask_size: Quantity) {
        if let Some(b) = &mut self.bid { b.update(bid); }
        if let Some(a) = &mut self.ask { a.update(ask); }
        self.last_bid_size = bid_size;
        self.last_ask_size = ask_size;
    }

    pub fn merge(&mut self, other: &QuoteBar) {
        if let (Some(b), Some(ob)) = (&mut self.bid, &other.bid) { b.merge(ob); }
        if let (Some(a), Some(oa)) = (&mut self.ask, &other.ask) { a.merge(oa); }
    ...
    pub fn from_lean_csv_line(line: &str, symbol: Symbol, date: chrono::NaiveDate, period: TimeSpan) -> Option<Self> {
        ...
        let parse = |s: &str| -> Option<Decimal> { s.trim().parse::<Decimal>().ok().map(|v| v / scale) };

        let bid = Bar::new(
            parse(parts[1])?, parse(parts[2])?, parse(parts[3])?, parse(parts[4])?,
        );
        let last_bid_size: Quantity = parts[5].trim().parse().unwrap_or(dec!(0));

        let ask = Bar::new(
            parse(parts[6])?, parse(parts[7])?, parse(parts[8])?, parse(parts[9])?,
        );
        let last_ask_size: Quantity = parts.get(10)
        ...
        Some(QuoteBar::new(symbol, time, period, Some(bid), Some(ask), last_bid_size, last_ask_size))
    }
}

impl BaseData for QuoteBar {
    fn data_type(&self) -> BaseDataType { BaseDataType::QuoteBar }
    fn symbol(&self) -> &Symbol { &self.symbol }
    fn time(&self) -> DateTime { self.time }
    fn end_time(&self) -> DateTime { self.end_time }
    fn price(&self) -> Price { self.mid_close() }
