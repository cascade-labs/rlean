...
use rlean_algorithm::margin_call::MarginCallOrderRequest;
use rlean_core::{DateTime, NanosecondTimestamp, Price, Symbol};
use rlean_orders::order::{OrderDirection, OrderStatus};
use rlean_orders::transaction_manager::TransactionManager;
use rlean_orders::{Order, OrderEvent, OrderTicket, UpdateOrderFields};
    ...
    items
        ...
        .filter_map(|(symbol, quantity)| {
            ...
            Some(MarginCallOrderRequest::new(
                Symbol::create_equity(&symbol, &rlean_core::Market::usa()),
                quantity,
            ))
        })
        .collect()
...
impl OrderEventView {
    ...
    fn py_utc_time(&self) -> chrono::NaiveDateTime {
        rlean_core::NanosecondTimestamp(self.utc_time_ns())
            .to_utc()
            .naive_utc()
    }
    ...
    #[getter(status)]
    fn py_status(&self) -> crate::types::OrderStatus {
        use rlean_orders::OrderStatus as Core;
    ...
    fn py_direction(&self) -> crate::types::OrderDirection {
        use rlean_orders::OrderDirection as Core;
