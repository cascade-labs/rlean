use rlean_algorithm::algorithm::{DataDeliveryPayload, QcAlgorithmStrategy, SecurityChanges};
...
impl QcAlgorithmNativeBridge {
    ...
    fn state(&self) -> Arc<std::sync::Mutex<rlean_algorithm::qc_algorithm::QcAlgorithm>> {
...
impl AlgorithmStateAccess for QcAlgorithmNativeBridge {
    fn algorithm_state(
        &self,
    ) -> Option<Arc<std::sync::Mutex<rlean_algorithm::qc_algorithm::QcAlgorithm>>> {
        Some(self.state())
    }
}
...
mod tests {
    use super::*;
    use rlean_algorithm::lifecycle::NoopAlgorithmServices;
    use rlean_algorithm::qc_algorithm::OptionFilter;
    use rust_decimal_macros::dec;

    struct QcBackedTestStrategy {
        state: Arc<std::sync::Mutex<rlean_algorithm::qc_algorithm::QcAlgorithm>>,
    }
        ...
        fn new() -> Self {
            Self {
                state: Arc::new(std::sync::Mutex::new(
                    rlean_algorithm::qc_algorithm::QcAlgorithm::new("native", dec!(100000)),
                )),
            }
        }
    }
    ...
    impl QcAlgorithmStrategy for QcBackedTestStrategy {
        fn algorithm_state(
            &self,
        ) -> Arc<std::sync::Mutex<rlean_algorithm::qc_algorithm::QcAlgorithm>> {
            self.state.clone()
        }

        fn initialize(&mut self) -> rlean_core::Result<()> {
