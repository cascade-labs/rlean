...
impl BacktestEngine {
    ...
    pub async fn run(
        &self,
        mut algorithm: Box<dyn IAlgorithm>,
    ) -> LeanResult<ResultHandler> {
        ...
        while current_date <= end_date {
            ...
            let bars_map: std::collections::HashMap<u64, lean_data::TradeBar> = slice.bars
                .iter()
                .map(|(k, v)| (*k, v.clone()))
                .collect();

            use chrono::{TimeZone, Utc};
            let utc_time = DateTime::from(Utc.from_utc_datetime(&current_date.and_hms_opt(16, 0, 0).unwrap()));
        ...
        info!("Backtest complete. {} trading days processed.", trading_days);
        Ok(result_handler)
    }
}
