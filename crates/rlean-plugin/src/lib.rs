/// rlean-plugin — C-stable plugin ABI for rlean
///
...
/// `Box<Arc<dyn rlean_data_providers::ICustomDataSource>>` and takes ownership.
///
/// A corresponding destroy function must also be exported:
/// ```rust,ignore
/// #[no_mangle]
/// pub unsafe extern "C" fn rlean_destroy_custom_data_source(ptr: *mut ()) {
///     drop(Box::from_raw(ptr as *mut Arc<dyn rlean_data_providers::ICustomDataSource>));
/// }
/// ```
///
/// Note: `ICustomDataSource` is defined in `rlean-data-providers`, not `rlean-plugin`,
...
///
/// Returns a heap-allocated `Box<Box<dyn rlean_data::DataQueueHandler>>` cast to
...
///
/// Returns a heap-allocated `Box<Box<dyn rlean_brokerages::Brokerage>>` cast to
/// `*mut ()`.
...
///
/// ```rust,ignore
/// use rlean_plugin::{PluginKind, PluginDescriptor, rlean_plugin};
///
/// rlean_plugin! {
///     name    = "tradier",
///     version = "0.1.0",
